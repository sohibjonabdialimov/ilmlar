...
const Cart = (props) => {
  ...
  useEffect(()=>{
    axios.get("http://64.226.118.71:5001/teacherinfo/"+props?.cart?.teacher_Id).then((res)=>{
      setTeacher(res.data);
    })
  },[])
  return (
    ...
    >
      <div className="main-cart" style={{cursor: "pointer"}}>
        ...
        <img src={urlJoin("http://64.226.118.71:5001", `${deleteplatforma(props?.cart?.obloshka)}`)} alt="" />
        <div className="cart-desc_wrap">
          ...
          <div className="desc">
            {
              teacher.path?<img className="small_img" src={urlJoin("http://64.226.118.71:5001", `${deleteplatforma( teacher.path)}`)} alt="" />:<img src={defaultuser}></img>
            }
